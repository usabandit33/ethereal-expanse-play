# Ethereal Expanse v28 Changelog
...
**Silent game / missing audio feedback.** Prior changelogs (v15, v25 notes) already flagged “no audio feedback for fire/escalation.” This is the single biggest remaining player-facing weakness: every action felt muted. (Later version numbers in the repo were tiny placeholders; v15 remained the only real full build.)
...
Hooks placed only on existing success paths (shootBeam, takeDamage, walker/boss hits, gem/coin/scale/pearl/wood collect, placeCamp, digHole, enter vehicle, fire breath, escalation). No new gameplay systems.
...
- 17 `sfx.*` call sites, all on existing success paths.

## Self-review scores (v28)

| Category          | Score | Notes |
|-------------------|-------|-------|
| Architecture      | 7     | Still monolithic; audio is a self-contained IIFE with named `sfx.*` helpers. |
| Maintainability   | 8     | Clear v28 markers; cues are one-liners at call sites. |
